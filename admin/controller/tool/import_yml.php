...
class ControllerToolImportYml extends Controller { 
  ...
  public function index() 
  {
    ...
    $data['save'] = $this->url->link('tool/import_yml/save', 'user_token=' . $this->session->data['user_token'], 'SSL');
    ...
    $data['breadcrumbs'][] = array(
      ...
      'href'      => $this->url->link('tool/import_yml', 'user_token=' . $this->session->data['user_token'], 'SSL'),
    );
    
    $data['action'] = $this->url->link('tool/import_yml', 'user_token=' . $this->session->data['user_token'], 'SSL');

    $data['export'] = $this->url->link('tool/import_yml/download', 'user_token=' . $this->session->data['user_token'], 'SSL');
    ...
    if (!empty($this->settings['import_yml_url'])) {
      ...
      if ($data['loaded']) {
        $this->session->data['success'] = sprintf(
          ...
          $this->url->link('tool/import_yml/cancel', 'user_token=' . $this->session->data['user_token'], 'SSL')
        );
  ...
  public function save()
  {
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
	  ...
	  $this->response->redirect($this->url->link('tool/import_yml', 'user_token=' . $this->session->data['user_token'], 'SSL'));
